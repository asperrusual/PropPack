...
function petBehavior.reactToMonster(entityId)
  local entityName = world.monsterType(entityId)
  local isToy = entityName:match('^toy') ~= nil
  if isToy or entityName == 'petball' then
    petBehavior.queueAction("play", {pounceTarget = entityId})
  end
end
...
function petBehavior.reactToObject(entityId)
  local entityName = world.entityName(entityId)
  -- Food bowls for pets should now have "petFoodBowl" : true, also backwards compatable with old name based system
  if entityName == "foodbowl" or world.getObjectParameter(entityId,"petFoodBowl",false) then
    local item = world.containerItemAt(entityId, 0)
    if item then
      local foodLiking = itemFoodLiking(item.name)
      --inspectaction.lua also checks if the name is "foodbowl" minor alterations are made there as well
      if foodLiking then
        local score = math.max(status.resource("hunger") - (100 - foodLiking), petBehavior.starvingLevel())
        petBehavior.queueAction("eat", {eatTarget = entityId}, score)
      elseif foodLiking == nil then
  ...
  --Either of these variables make a bed sleepable like the old pethouse, they each indicate a different default sleep position and sleep animation
  
  local petBed   = world.getObjectParameter(entityId,"category","") == "petbed"
  local petHouse = world.getObjectParameter(entityId,"category","") == "pethouse"
  
  --also backwards compatible TODO remove and overwrite /objects/generic/petbed.object
  
  petHouse = entityName == "pethouse" and true or petHouse


  local pb = petBed and "petBed" or "not petBed"
  local ph = petHouse and "petHouse" or "not petHouse"

    
  --NEW TODO use messaging system external function to get type of furniture

  local sleepSpot = petBed and {4,16} or {16,16}
  local sleepAnimation = petHouse and "invisible" or "sleep"


    if petBed or petHouse then 
      petBehavior.queueAction("sleep", 
          {sleepTarget = entityId, 
           sleepAnimation = petHouse and "invisible" or "sleep", 
           sleepSpot = world.getObjectParameter(entityId,"petPosition", sleepSpot) })
    end
